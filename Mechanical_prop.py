...
def impro_integrationv2(f,n):
    # Integrate fun_to_int from x=0 to x=cutoff, then from x=cutoff to x=infinity.
    cutoff = 4.0
    #x1=np.linspace(0,cutoff,n)
    
    #x2=np.linspace(0,cutoff**-1,n)
    f2=lambda y:f(y**-1)*(y**-2)
    
    Int1=Rdsimpson13(f,0,cutoff,n)
    Int2=Rdsimpson13(f2,1e-9,1.0/cutoff,n)
...
def doubleintegral_simpson(function_phi_theta,phi,theta):

# Compute the double integral of function funcion_phi_theta by the Simpson
# method
#
# INPUT:
# funcion_phi_theta -> Array of valued points of the funtion to be 
# integrated (f(x,y))
# phi -> Array of phi values (x)
# theta -> Array of theta values (y)
# 
...
def int_mat(M_to_int,phi,theta):
    ...
    for i in range(d1):
        for j in range(d2):
            Mav[i,j]=doubleintegral_simpson(np.reshape(M_to_int[i,j,:,:],[length_phi,length_theta]),phi,theta);
    return Mav

def computeEshelbyIntegrals(a):
    ...
    A[0,0]=  c*impro_integrationv2(F1,n) 
    A[1,0] = c*impro_integrationv2(F2,n) 
    ...
    B[0,0] = c*impro_integrationv2(F11,n) 
    B[1,1] = c*impro_integrationv2(F22,n) 
    B[2,2] = c*impro_integrationv2(F33,n) 
    B[0,1] = c*impro_integrationv2(F12,n) 
    B[0,2] = c*impro_integrationv2(F13,n) 
    B[1,2] = c*impro_integrationv2(F23,n) 
    B[1,0] = B[0,1]
    B[2,0] = B[0,2]
    B[2,1] = B[1,2]
    return np.mat(A),np.mat(B)
        
def eshelby_int(a,nu):
...
def rotmatrix(Beta,Alpha,Psi,var):
    ...
    R[0,:,:] = [[1,0,0],[0,np.cos(np.radians(Beta)),np.sin(np.radians(Beta))],[0,-np.sin(np.radians(Beta)),np.cos(np.radians(Beta))]]
    R[1,:,:] = [[np.cos(np.radians(Alpha)),0,-np.sin(np.radians(Alpha))],[0,1,0],[np.sin(np.radians(Alpha)),0,np.cos(np.radians(Alpha))]]
    R[2,:,:] = [[np.cos(np.radians(Psi)),np.sin(np.radians(Psi)),0],[-np.sin(np.radians(Psi)),np.cos(np.radians(Psi)),0],[0,0,1]]
    ...
    K1 = np.array([[w[0,0]**2,w[0,1]**2,w[0,2]**2],[w[1,0]**2,w[1,1]**2,w[1,2]**2],[w[2,0]**2,w[2,1]**2,w[2,2]**2]])
    K2 = np.array([[w[0,1]*w[0,2],w[0,2]*w[0,0],w[0,0]*w[0,1]],[w[1,1]*w[1,2],w[1,2]*w[1,0],w[1,0]*w[1,1]],[w[2,1]*w[2,2],w[2,2]*w[2,0],w[2,0]*w[2,1]]])
    K3 = np.array([[w[1,0]*w[2,0],w[1,1]*w[2,1],w[1,2]*w[2,2]],[w[2,0]*w[0,0],w[2,1]*w[0,1],w[2,2]*w[0,2]],[w[0,0]*w[1,0],w[0,1]*w[1,1],w[0,2]*w[1,2]]])    
    K4 = np.array([[w[1,1]*w[2,2]+w[1,2]*w[2,1],w[1,2]*w[2,0]+w[1,0]*w[2,2],w[1,0]*w[2,1]+w[1,1]*w[2,0]],[w[2,1]*w[0,2]+w[2,2]*w[0,1],w[2,2]*w[0,0]+w[2,0]*w[0,2],w[2,0]*w[0,1]+w[2,1]*w[0,0]],[w[0,1]*w[1,2]+w[0,2]*w[1,1],w[0,2]*w[1,0]+w[0,0]*w[1,2],w[0,0]*w[1,1]+w[0,1]*w[1,0]]])
...
def rottensor(Beta,Alpha,Psi,order,A,val):
    ...
    if val==1: # Stiffness tensor
         ...
         #A_rot=(T1)*A*(W*T1*invW)';
         A_rot=T1*(A*np.linalg.inv(T1))
    return A_rot  

def softinterphase(kappa,lambd,vp):
    if vp>=1:
        vi=0
    else:
        if kappa>1:
    ...
    m=0
    #exponent=-(6*vp/(1-vp))*(lambd/nkappa+(2+3*vp/((nkappa**2)*(1-vp)))*(lambd**2)+(4/3)*(1+3*vp/(nkappa*((1-vp)**2))+m*(vp**2)/((nkappa**3)*((1-vp)**2)))*(lambd**3));
    #exponen = np.exp(exponent);  
    vi=(1-vp)*(1-np.exp(-(6*vp/(1-vp))*(lambd/nkappa+(2+3*vp/((nkappa**2)*(1-vp)))*(lambd**2)+(4./3.)*(1+3*vp/(nkappa*((1-vp)**2))+m*(vp**2)/((nkappa**3)*((1-vp)**2)))*lambd**3)))
    return vi        

def orientational_average(number_theta,number_phi,theta_max,phi_max,A,k,val):
    ...
    if k>1000000:
        ...
        for t in range(length_theta):
            ...
            for s in range(length_phi):
                phi_loop=phi[s]
                A_phi_theta[:,:,s,t]=rottensor(theta_loop,phi_loop,0,[2,0,1],A,val)
                
                A_phi_theta[:,:,s,t]=rottensor(0,phi_loop,theta_loop,[0,2,1],A,val)   
                
            denom_phi_theta[0,0,:,t]=(ODF[t]*np.sin(theta_loop))
            A_to_int[:,:,:,t]=np.reshape(A_phi_theta[:,:,:,t],[6,6,len(phi)])*ODF[t]*np.sin(theta_loop)                
...
def ellipsoidalinter_random2(Cp,Ci,Cm,Si,Sp,vp,vi):
    if vi==0:
        number_theta=20
        number_phi=20
        theta_max=np.pi/2
        ...
        if vp==0:
            ...
            Mp=-np.linalg.inv(Sp+psiP+((dS*(factor1))*np.linalg.inv(factor2)))
            Mi=-np.linalg.inv(dS+(factor3*factor2))
            Adil1=Id+(Sp*Mp)+(dS*Mi)
            Adil2=Id+(Si*Mi)+(vp/vi)*(dS*(Mp-Mi))
            Ceff=(Cm)
            Ceff = orientational_average(number_theta,number_phi,theta_max,phi_max,Ceff,0,1)
    else: 
        #Case 2
        number_theta=20
        number_phi=20
        ...
        if vp==0:
            ...
            Ceff=((vm*Cm+vi*(Ci*Adil2)+vp*(Cp*Adil1))*np.linalg.inv(vm*Id+vi*Adil2+vp*Adil1))
            
            Ceff=orientational_average(number_theta,number_phi,theta_max,phi_max,Ceff,0,1)
            
    return Ceff       
def computeEngineeringConstantsSqrt2(C):
    ...
    S = np.mat(np.linalg.inv(C)) # Inverse of a symmetric matrix is symmetric
    ...
    nu[5] = nuscale[1,0]
    return E, nu
def computemechanicalprops(E_matrix,nu_matrix,E_cnt,nu_cnt,E_i,nu_i,\
                           L_cnt,D_cnt,inter_t,type_inter,vf):

    Cm = isotropic(E_matrix,nu_matrix)
    Cp = isotropic(E_cnt,nu_cnt)
    #Interphase
    Kappa = L_cnt/D_cnt
    Deq = D_cnt*Kappa**(1./3.) 
    Ci = isotropic(E_i,nu_i)
    Sp = eshelby_int([1,Kappa,1],nu_matrix)
    Si = eshelby_int([1,Kappa,1],nu_matrix)
    #if typeinter==1
    lambd = inter_t/Deq
    vi= softinterphase(Kappa,lambd,vf)
    Ceff= ellipsoidalinter_random2(Cp,Ci,Cm,Si,Sp,vf,vi)

    E,nu = computeEngineeringConstantsSqrt2(Ceff)
    
    return E[0],nu[0]
#Matrix
E_matrix = 2.5e9             # Young´s modulus MPa  
nu_matrix = 0.28               # Poisson´s ratio
#densm = 1.12               # Density g/cm3

# MWCNTs 
#Isotropic material
E_cnt=700e9                # Young´s modulus MPa  
nu_cnt=0.3                  # Poisson´s ratio
...
vf=0.01
E,nu=computemechanicalprops(E_matrix,nu_matrix,E_cnt,nu_cnt,E_i,nu_i,L_cnt,D_cnt,inter_t,type_inter,vf)
